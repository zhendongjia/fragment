...
C      ZMOON = SCALE/(81.0*330000.0)
C      ZMOON = 161.0*ZMOON/DFLOAT (N)
      ZMOON = 3.694E-8
      M_EARTH = 3.000E-6
...
C      RM = 1.15E-05*(161.0*SCALE/DFLOAT (N))**0.3333
      RM = 1.158E-05
...
C          INCLUDE ONE INITIAL BODY WITH SPECIFIED MASS & ORBITAL PARAMETERS.
      DO K=1,NJUPITER
         ...
         WRITE (6,29)  BODY(N),SEMI(N),ECC(N)
...
C     READ GAS DISK PARAMETER
 30   IF (KZ(18).NE.0) THEN
         READ (5,*) T_DEP, R_EDGE, R_IN, DENS0
         WRITE (6,31) T_DEP, R_EDGE, R_IN, DENS0
 31   FORMAT (//,5X,'GAS DISK PARAMETERS',F12.0,3F10.4)
      END IF
      READ (5,*) CRIT_STEP, CRIT_DISTANCE, ESCAPE_DISTANCE
      CRIT_DISTANCE2 = CRIT_DISTANCE**2
      ESCAPE_DISTANCE2 = ESCAPE_DISTANCE**2
C
