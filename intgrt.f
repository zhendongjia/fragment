...
C
      INCLUDE 'COMMONP.FOR'
      COMMON/CM/  XCM(3),VCM(3),BCM,BMAX,XF(8),VF(8),BF(8),AI,
     &		  NF,IF(10),ICOMP,JCOMP
      REAL*4  RSMIN2,RSMAX2,R3INV,RSEP2,BODYJ,FRAGM
      COMMON/TABLE/  RSMIN2,RSMAX2,R3INV(0:10000)
      REAL*4  F2DOT(4),WK(10)
      REAL*8  FP(3),F1(4),F1DOT(4)
C
C
...
C          INCLUDE ALL EMBRYOS INSIDE THE SPECIFIED PERTURBER BINS.
      BODYJ = BODY(J)
      IF (BODYJ.GT.EMBRYO)  GO TO 32
      IF (IDIS.GT.MPERT(J).OR.IABS (IBINR-LISTR(J)).GT.NRPERT)  GO TO 34
C
...
C          CHECK APPROACHING FRAGMENT (IGNORE VERTICAL MOTION).
      IF (BODYJ.GT.FRAGM)  GO TO 34
      RDOT = XIJ*(X0DOT(1,J)-X0DOT(1,I)) + YIJ*(X0DOT(2,J)-X0DOT(2,I)) 
      IF (RDOT.GT.0.0)  GO TO 34
      DELT = RIJ2/RDOT
      IF (DELT.LT.CHECK)  GO TO 34
      CHECK = DELT
      JMIN = J
   34 IF (L.LE.NNB)  GO TO 30
C
   35 IBINJ = IBINJ + 1
