...
C      SPIN(ICOMP) = SPIN(ICOMP) + BODY(ICOMP)*BODY(JCOMP)*
C     &       ((X(1,ICOMP)-X(1,JCOMP))*(XDOT(1,ICOMP)-XDOT(1,JCOMP)) +
C     &        (X(2,ICOMP)-X(2,JCOMP))*(XDOT(2,ICOMP)-XDOT(2,JCOMP)))/BCM
C
C          MOVE REMNANTS TO C.M. FOR ENERGY CALCULATION (MERGED LATER).
C      DO 10 K = 1,3
C      X(K,ICOMP) = XCM(K)
C      X(K,JCOMP) = XCM(K)
C      XDOT(K,ICOMP) = VCM(K)
C      XDOT(K,JCOMP) = VCM(K)
C   10 CONTINUE
