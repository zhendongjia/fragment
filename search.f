...
C          PREDICT CURRENT COORDINATES AND VELOCITIES FOR BODY JCOMP.
      ...
      A1 = 0.05D0*DT
      A2 = 0.25D0*DT
      A3 = (T0(JCOMP) - T1(JCOMP)) + (T0(JCOMP) - T2(JCOMP))
C
      RIJ2 = 0.0
      VREL2 = 0.0
      RDOT = 0.0
...
C          EVALUATE OSCULATING SEMI-MAJOR AXIS AND ECCENTRICITY.
      ...
      SEMI = 2.0/RIJ - VREL2/BCM
      SEMI = 1.0/SEMI
      ECC = DSQRT ((1.0 - RIJ/SEMI)**2 + RDOT**2/(SEMI*BCM))
...
CC    GAMMA = DSQRT (GAMMA2)
      ...
      WRITE (6,50)  NAME(I),NAME(JCOMP),YEARS,RIJ,SEMI,ECC,PERI,GAMMA, 
     &                                                    GAMMAS,STEP(I)
...
C          CHECK MERGER CRITERION FOR STABLE BINARY (ICASE = 2).
      IF (SEMI.LT.0.0)  GO TO 100
      PERIOD = SEMI*DSQRT (SEMI/BCM)
C          DEFINE CLOSE BINARY IF PERIOD < 0.05 YEARS.
      IF (PERIOD.GT.0.05)  GO TO 100
      WRITE (6,65)  NAME(I),NAME(JCOMP),SEMI,PERI,ECC,PERIOD
...
C          CHECK COLLISION CONDITION.
   ...
   75 ICOMP = MIN (I,JCOMP)
      JCOMP = MAX (I,JCOMP)
C          ROUTINE MERGE ASSUMES ICOMP < JCOMP.
C
      EPAIR = -BODY(ICOMP)*BODY(JCOMP)/(2.0*SEMI)
...
CC    GAMMA = DSQRT (GAMMA2)
      GAMMA = 0.0
      VCOLL = BCM*(2.0/(R(ICOMP) + R(JCOMP)) - 1.0/SEMI)
     ...
     &                               BODY(JCOMP)/(BODY(ICOMP)*R(JCOMP)))
      ...
      ACRAT = VREB/VESC
      WRITE (6,85)  NAME(ICOMP),NAME(JCOMP),YEARS,AMASS1,AMASS2,AMASS3,  
     &                   RIJ,SEMI,ECC,PERI,GAMMA,EPAIR,RATIO,PERRI,ACRAT
   85 FORMAT (3X,'COLLISION',2I4,F9.1,3F5.1,F11.6,F10.5,F10.4,F12.6,
     &                                        F7.1,1PE10.1,0PF8.2,2F7.3)
C
