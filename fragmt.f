...
C      ZMEJ = MF(1)*NMF(1)
C      DO 10 L =2, LF
C         ZMEJ = ZMEJ + MF(L)*(NMF(L) - NMF(L-1))
C 10   CONTINUE
C      BODY(ICOMP) = (MTOT - ZMEJ)/2
C      BODY(JCOMP) = (MTOT - ZMEJ)/2
C      CALL UPDATE_ORBIT(ICOMP)
C      CALL UPDATE_ORBIT(JCOMP)
C      R(ICOMP) = RI*(BODY(ICOMP)/BODYI)**0.3333
C      R(JCOMP) = RI*(BODY(JCOMP)/BODYI)**0.3333
C      EGRAV = EGRAV + 0.6*BODY(ICOMP)**2/R(ICOMP) 
C     &     + 0.6*BODY(JCOMP)**2/R(JCOMP)
C
...
C      SPIN(ICOMP) = SPIN(ICOMP)*BODY(ICOMP)/BODYI
C      SPIN(JCOMP) = SPIN(JCOMP)*BODY(JCOMP)/BODYI
      NSTEPN(9) = NSTEPN(9) + 1
C
      RETURN
C
