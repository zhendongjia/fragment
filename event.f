...
C          SET ORBITAL CHARACTERISTICS OF FRAGMENTS AND MERGE CORES.
   50 IF (NF.GT.0)  THEN
          CALL INFRAG
          CALL REMOVE(ICOMP)
          IF (JCOMP.GT.ICOMP) JCOMP = JCOMP - 1
          CALL REMOVE(JCOMP)
      END IF
C
...
C          ENFORCE MERGER IF SEMI-MAJOR AXIS IS INSIDE SUM OF RADII.
      ...
      SEMIREL = (VESC**2 - CII**2*VREL2)/BCM
      SEMIREL = 1.0/SEMIREL
      IF (SEMIREL.GT.0.0.AND.SEMIREL.LT.R(ICOMP) + R(JCOMP))  THEN
          WRITE (6,60)  ICOMP,JCOMP,SEMIREL,R(ICOMP)+R(JCOMP)
...
C          PRINT ORBITAL ELEMENTS OF THE CRATERED BODY.
      ...
      WRITE (6,80)  NAME(I),DABS (DMC)/ZMOON,SEMI(I),ECC(I),RI
